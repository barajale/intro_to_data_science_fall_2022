...
using CSV ,DataFrames, Printf, CairoMakie, ColorSchemes, PlutoUI, RDatasets, Statistics, Random

# ╔═╡ de0b0cae-d0dd-4a23-9acc-7a72b6c5e0dd
md"Alexander Barajas-Ritchie

hw partner: Cameron Irmas"
...
begin
	download("https://raw.githubusercontent.com/data-8/textbook/main/assets/data/bta.csv", "data.csv")
...
group_df = combine(groupby(data, "Group"), 
	"Result" => mean => "Fraction with pain relief")
...
begin
	colors = reverse([get(ColorSchemes.candy, i, (1, 2)) for i = 1:2])
	fig = Figure()
	ax = Axis(fig[1,1],
		title="Experiment group comparison",
		xlabel="Group",
		ylabel="proportion of patients with reduced pain",
		xticks=(1:nrow(group_df), group_df[:, "Group"]),
		yticks=(collect(Float64,0:.1:1))
	)
	barplot!(1:nrow(group_df), group_df[:,"Fraction with pain relief"], 
		color=colors,
...
md"
_null hypothesis_: there is no difference in pain relief between the two groups

_alternative hypothesis_: there is a difference in pain relief in the groups
"
...
md"
_test statistic = |0.6 - 0.125| = 0.475
"
...
transform!(data,"Group" =>  
	(col -> shuffle!(data[:,"Group"])) 
		=> :"Sim Group under Null")
...
function proportion_w_pain_relief(data, group_col, group)
	mean(filter(row -> row[group_col] == group, data)[:,"Result"])
end
...
proportion_w_pain_relief(data, "Group", "Treatment")

# ╔═╡ 18fd84e0-0e57-11eb-36a6-c72f5f1db6a6
proportion_w_pain_relief(data, "Group", "Control")
...
function test_statistic(data, group_col)
	return abs(
		proportion_w_pain_relief(data, group_col, "Control") - proportion_w_pain_relief(data, group_col, "Treatment")
	)
end
...
observed = test_statistic(data, "Group")
...
# ╔═╡ c2de753f-81d3-423f-99d7-b1b2c2ccc5ad
md"A"

# ╔═╡ 1438de8a-0e59-11eb-2bff-effebb5c898b
begin
		test_stats = zeros(10000)
		for i in eachindex(test_stats)
			transform!(data,"Group" =>  
				(col -> shuffle(data[:,"Group"])) 
					=> :"Sim Group under Null")
			test_stats[i] = test_statistic(data, "Sim Group under Null")
		end
end

# ╔═╡ 18cbf505-a472-4f8b-95c2-1f0a86583541
md"B"

# ╔═╡ 553a3afc-d17b-48a4-8200-afd87efee7ab
begin
	local fig = Figure()
	local ax = Axis(fig[1,1], 
		title="Prediction Under the Null Hypothesis", 
		xlabel="Distance",
	    ylabel="Percent per unit",
		xticks=(collect(Float64, 0:0.1:0.6))
	)

	hist!(ax, test_stats, bins = collect(Float64, 0:0.1:0.6), color="green",strokecolor=:black, strokewidth=1)
	vlines!(ax, [observed], color="red", linewidth=2)
	text!((observed)+.01, 4500,text="Test Stat: $observed")
	ylims!(0, nothing)
	xlims!(0,nothing)
	p_value = size(filter(item -> item >= observed, test_stats),1) / size(test_stats,1)
	
	text!((p_value * 100)- .2, 4500,text="p value: $(p_value *100)")
	vlines!(ax, [p_value * 100], color="blue", linewidth=2)
	fig
end
...
# ╔═╡ 3cf71f3f-e45e-45ec-b73f-6d73101af37e
md"Given the P value above, we can see that there is a statistical difference in the groups. we can reject the null hypothesis based on the simuation"
...
I choose a one side hypothesis due to the nature of the distrubtion.
...
no, we can only conclude a difference in the groups not that the group caused pain relief.
...
If a patient was allowed to choose, we wouldn't be able to control for the placebo effect. When we lose the random nature of the experience it's difficult to be certain.
"
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "01a846d33e2c35d488829bdd76ebafc68f15be24"
